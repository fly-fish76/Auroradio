...
kw / 咪咕等需要脚本解析的音源，按以下步骤启用：

1. 前往 [fly-fish76/lx-source](https://github.com/fly-fish76/lx-source) 下载你想要的音源脚本（`.js` 文件）；
2. 在软件内打开 **设置 → 音源设置**，选择 **导入自定义音源脚本**，选中下载的 `.js` 文件；
3. 导入并启用后，即可在搜索、歌单中使用对应音源，无需登录对应平台。

音源宿主（脚本运行环境）已内置在 Auroradio 主程序中，无需单独安装。音源脚本均收集自公开渠道、版权归原作者所有，可用性取决于脚本作者维护的接口。
